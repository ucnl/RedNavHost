...
namespace RedNAVHost
{
    public partial class MainForm : Form
    {
        ...
        List<RNGeoPoint2D> waypoints = new List<RNGeoPoint2D>();
        ...
        private void waypointsLbx_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                RNGeoPoint2D pnt = (RNGeoPoint2D)waypointsLbx.SelectedItem;
        ...
        private void changePntBtn_Click(object sender, EventArgs e)
        {
            if (waypointsLbx.SelectedItem != null)
            {
                RNGeoPoint2D pnt = new RNGeoPoint2D(latDeg + latMin / 60.0 + latSec / 3600.0, 
        ...
        private void addWaypoint_Click(object sender, EventArgs e)
        {
            ...
            {
                waypointsLbx.Items.Add(new RNGeoPoint2D(0, 0, 2));
        ...
        private void port_ACKReceived(object sender, EventArgs e)
        {
            if (isClearingWaypoints)
            {
                ...
                if (waypointsLbx.Items.Count > 0)
                {
                    foreach (RNGeoPoint2D item in waypointsLbx.Items)
                    {
                        if (item.PType != 0)
                        {
        ...
        private void port_TrackItemReceived(object sender, TrackItemEventArgs e)
        {
            ...
            {
                var result = trkFilter.Filter(e.Latitude, e.Longitude);
                track.Add(new TrackItem(e.TimeStamp, result.Latitude, result.Longitude, e.Depth));
            }
        ...
        {
            RNGeoPoint2D pnt = new RNGeoPoint2D(e.Latitude, e.Longitude, e.WaypointType);
