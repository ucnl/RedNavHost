...
namespace RedNAVHost
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            //
            //var lines = File.ReadAllLines("C:\\temp\\00\\0.csv");
            //var sep = new char[] { ';' };

            //track = new List<TrackItem>();
            ...
            //    {
            //        var ln = line.Replace(',', '.');
            //        var splits = ln.Split(sep, StringSplitOptions.RemoveEmptyEntries);
                    
            //        var lat = double.Parse(splits[1], CultureInfo.InvariantCulture);
            //        var lon = double.Parse(splits[2], CultureInfo.InvariantCulture);
            //        var dpt = double.Parse(splits[3], CultureInfo.InvariantCulture);

            //        track.Add(new TrackItem(DateTime.Now, lat, lon, dpt));
            //    }
            //}

            //KMLData data = new KMLData("RedWAVE track", "Generated by RedNavHost, www.unavlab.com");
            ...
            //TinyKML.Write(data, "C:\\temp\\00\\track.kml");
        ...
        */
        ...
        private void waypointsLbx_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (waypointsLbx.SelectedItem != null)
            {
                ...
                if (pnt.PType == 2)
                {
                    ...
                    latMin = (int)Math.Round((pnt.Latitude - latDeg) * 60.0);
                    double tmp = (pnt.Latitude - latDeg) * 3600.0 - latMin * 60.0;
                    if (tmp < 0) tmp = 0;
                    latSec = tmp;

                    lonDeg = (int)Math.Floor(pnt.Longitude);
                    lonMin = (int)Math.Round((pnt.Longitude - lonDeg) * 60.0);
                    tmp = (pnt.Longitude - lonDeg) * 3600.0 - lonMin * 60.0;
                    if (tmp < 0) tmp = 0;
                    lonSec = tmp;
        ...
        private void port_ConnectionSearchingStarted(object sender, EventArgs e)
        {
            ...
            InvokeSetText(deviceVersionLbl, "- - -");      
            ...
            InvokeSetEnabledState(waypointsLbx, false);
        ...
        private void port_PortReady(object sender, EventArgs e)
        {
            ...
            InvokeSetText(deviceVersionLbl, string.Format("{0} v{1}, {2}: {3} v{4}",                
                port.SystemMoniker, 
                port.SystemVersion, 
            ...
            InvokeSetEnabledState(waypointsLbx, true);
        ...
        private void port_RNDeviceInfoUpdated(object sender, EventArgs e)
        {
            ...
            {
                InvokeSetText(redNodeInfoLbl, LocRM.GetString("mfText_NoDataStr"));
                InvokeSetText(redNodeSerialLbl, LocRM.GetString("mfText_NoDataStr"));
            }
            else
            {
                InvokeSetText(redNodeInfoLbl, string.Format("{0} v{1}, {2}: {3} v{4}",                     
